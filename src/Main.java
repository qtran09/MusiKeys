	...
	public void start(Stage stage) throws Exception {
		...
		bind.setOnAction(e -> {
			if(!currKey.getText().equals("")){
				...
				try{
					if(!extension.equals("wav") && !extension.equals("mp3")) throw new NullPointerException();//throw new Exception("ya");
		...
		bindAll.setOnAction(e -> { //Clears key and selection
			if(!currKey.getText().equals("")){
				...
				try{
					if(!extension.equals("wav") && !extension.equals("mp3")) throw new Exception();//throw new Exception("ya");
					if(sound != null && !sound.getName().equals("")){
						for(Button x: arrB){
							x.setStyle("-fx-background-color: burlywood");
